"""
voligle_A — production dashboard for Candidate A (locked).
...
LOCKED STRATEGY:
  ...
  Anchor      = after_smt
  Rule        = close_break
  TP          = TP1  (managed mode ON)
  Sessions    = london + ny_am
  Levels      = ALL except AsiaON High/Low, PWH, PWL
  Retracement = 0.50R (wait for retracement to fill)
  Risk        = $250 per trade
  Starting    = $25,000

Expected baseline (from compare_strategies sweep):
  ~171 trades · 86% WR · ~$36k PnL · max DD ~$750

Run:
  streamlit run voligle_A.py --server.port 8510
"""
...
TP_LEVEL         = 1                   # TP1
TP_COL_RAW       = "r_tp1"
TP_COL_MGD       = "r_tp1_mgd"
HIT_COL          = "hit_1r"
HIT_TS_COL       = "hit_1r_ts"
SESSIONS         = ["london", "ny_am"]
EXCLUDE_LEVELS   = {"AsiaONHigh", "AsiaONLow", "PWH", "PWL"}
ENTRY_RETRACE_R  = 0.50                # 0.50R retracement
...
st.set_page_config(page_title="voligle A", layout="wide")
...
# ============================================================
# RETRACEMENT SIMULATION (locked at ENTRY_RETRACE_R)
# ============================================================
@st.cache_data
def load_1m_bars():
    out = {}
...
def simulate_retraced_entries(trades_df, retrace_r, tp_multiplier, tp_col_name, bars_1m_by_asset):
    """Wait for price to retrace `retrace_r * R` after MSS; fill at that level.
    SL unchanged; TP = new_entry ± tp_multiplier * new_R. Trades that never
    retrace OR hit SL/kill before the retracement are skipped entirely.
    Preserves Phase 4 kill timestamps (opp_smt_kill, opp_mss_kill, smt_invalidation_kill).
    """
    if retrace_r == 0:
        out = trades_df.copy()
    ...
    for _, t in trades_df.iterrows():
        ...
        try:
            sub = bars_1m_by_asset[asset].loc[mss_ts + pd.Timedelta(minutes=1):sess_end]
        ...
        outcome_r = None; outcome_reason = None; outcome_exit_ts = None
        ...
        if outcome_r is None:
            if len(sub2) > 0:
                ...
                outcome_r = 0.0; outcome_exit_ts = fill_ts
...
with st.spinner(f"Simulating {ENTRY_RETRACE_R:.2f}R retraced entries..."):
    df = simulate_retraced_entries(df, ENTRY_RETRACE_R, float(TP_LEVEL), TP_COL, bars_1m)
n_skipped_no_retrace = n_before_sim - len(df)

if len(df) == 0:
    st.error(f"No trades retraced to {ENTRY_RETRACE_R:.2f}R. Strategy yields zero trades.")
    st.stop()

# Override columns so all downstream logic sees the simulated trades
# But preserve the ORIGINAL MSS bar time for chart visualization
...
df[HIT_COL] = (df["outcome_r"] >= float(TP_LEVEL) - 0.001)
...
# Page title
st.markdown("# 🔒 voligle_A — Production Strategy Dashboard")
...
st.markdown(
    ...
    <div style='background:#0e2030;border:1px solid #1976d2;border-radius:8px;padding:12px 16px;margin-bottom:12px'>
      <div style='display:flex;flex-wrap:wrap;gap:24px;font-size:14px'>
        ...
        <div><span style='color:#888'>Retrace:</span> <b>{ENTRY_RETRACE_R:.2f}R</b></div>
      ...
      <div style='color:#888;font-size:12px;margin-top:8px'>
        ...
        &nbsp;·&nbsp; Trades after retracement filter: {len(df)} (of {n_before_sim}, {n_skipped_no_retrace} skipped)
      </div>
    </div>
    """,
...
with dl1:
    st.download_button(
        ...
        file_name="voligle_A_trades.csv",
        mime="text/csv",
        use_container_width=True,
    )
with dl2:
    st.download_button(
        ...
        file_name="voligle_A_summary.csv",
        mime="text/csv",
        use_container_width=True,
    )
with dl3:
    st.download_button(
        ...
        file_name="voligle_A_monthly.csv",
        mime="text/csv",
        use_container_width=True,
    )
...
def show_trade_chart(trade):
    ...
    # Determine exit_ts first (needed for Option C X-range)
    ...
    # ============================================================
    # Option C zoom: MSS - 4h to exit + 1h
    # ============================================================
    x_start = mss_ts - pd.Timedelta(hours=4)
    x_end = exit_ts + pd.Timedelta(hours=1)

    mnq_bars_15m = load_bars("MNQ", TF).loc[x_start:x_end]
    mes_bars_15m = load_bars("MES", TF).loc[x_start:x_end]

    # 5m bars on the PICKED asset for the bottom row (position tool will live here)
    picked_5m = load_bars(measured_asset, "5m").loc[x_start:x_end]

    # MSS swing info — the swing pivot that got broken to trigger MSS
    swing_ts = trade.get("swing_ts")
    swing_price = trade.get("swing_price")
    have_mss_line = pd.notna(swing_ts) and pd.notna(swing_price)
    if have_mss_line:
        swing_ts = pd.Timestamp(swing_ts)
        swing_price = float(swing_price)

    # ============================================================
    # 2-row layout
    #   Row 1: MNQ 15m | MES 15m       (context — both assets at strategy TF)
    #   Row 2: Picked-asset 5m (full)  (entry drill-down — position tool here)
    # ============================================================
    fig = make_subplots(
        rows=2, cols=2, horizontal_spacing=0.04, vertical_spacing=0.08,
        row_heights=[0.45, 0.55],
        specs=[[{}, {}], [{"colspan": 2}, None]],
        subplot_titles=(
            f"MNQ · {TF}",
            f"MES · {TF}",
            f"{measured_asset} · 5m  (entry drill-down)",
        ),
    )

    def add_candles(d, row, col):
    ...
    # Row 1
    add_candles(mnq_bars_15m, 1, 1)
    add_candles(mes_bars_15m, 1, 2)
    # Row 2 (picked asset, 5m)
    add_candles(picked_5m, 2, 1)

    # Position tool lives on the 5m panel (row 2, col 1)
    pos_row, pos_col = 2, 1

    if pd.notna(mgd_exit_ts):
        try:
            bars_1m_a = load_bars(measured_asset, "1m")
            exit_price = bars_1m_a.loc[mgd_exit_ts]["close"]
    ...
    # ============================================================
    # Option Y: extend position tool right edge by 20% of trade duration
    # (with a 30-min minimum) so labels don't look cut off.
    # ============================================================
    trade_duration = exit_ts - mss_ts
    extension = max(trade_duration * 0.20, pd.Timedelta(minutes=30))
    pos_right = exit_ts + extension
    ...
    # SL + TP rectangles on the 5m panel
    fig.add_shape(type="rect", x0=mss_ts, x1=pos_right, y0=sl_bot, y1=sl_top,
                   fillcolor="rgba(239,83,80,0.25)", line=dict(width=0),
                   row=pos_row, col=pos_col)
    fig.add_shape(type="rect", x0=mss_ts, x1=pos_right, y0=tp_bot, y1=tp_top,
                   fillcolor="rgba(38,166,154,0.20)", line=dict(width=0),
                   row=pos_row, col=pos_col)
    fig.add_shape(type="line", x0=mss_ts, x1=pos_right, y0=entry, y1=entry,
                   line=dict(color="#1976d2", width=1.5),
                   row=pos_row, col=pos_col)
    fig.add_shape(type="line", x0=mss_ts, x1=pos_right, y0=sl, y1=sl,
                   line=dict(color="#d32f2f", width=1.5),
                   row=pos_row, col=pos_col)
    for n, rv in r_levels:
        fig.add_shape(type="line", x0=mss_ts, x1=pos_right, y0=rv, y1=rv,
                       line=dict(color="#388e3c", width=1, dash="dash"),
                       row=pos_row, col=pos_col)
    label_offset = pd.Timedelta(minutes=2)
    fig.add_annotation(x=pos_right + label_offset, y=entry,
                        text=f"Entry {entry:.2f}", showarrow=False,
                        font=dict(size=9, color="#1976d2"),
                        xanchor="left", yanchor="middle",
                        row=pos_row, col=pos_col)
    fig.add_annotation(x=pos_right + label_offset, y=sl,
                        text=f"SL {sl:.2f}", showarrow=False,
                        font=dict(size=9, color="#d32f2f"),
                        xanchor="left", yanchor="middle",
                        row=pos_row, col=pos_col)
    for n, rv in r_levels:
        fig.add_annotation(x=pos_right + label_offset, y=rv,
                            text=f"{n}R", showarrow=False,
                            font=dict(size=9, color="#388e3c"),
                            xanchor="left", yanchor="middle",
                            row=pos_row, col=pos_col)

    # Vertical dotted line at actual exit on the 5m panel
    exit_color = {"sl_hit":"#d32f2f","tp_hit":"#388e3c",
                   "session_end":"#757575","opp_smt_kill":"#9c27b0",
                   "opp_mss_kill":"#9c27b0","smt_invalidation_kill":"#9c27b0",
                   "kill":"#9c27b0"}.get(outcome, "#757575")
    fig.add_shape(type="line", x0=exit_ts, x1=exit_ts, y0=sl_bot, y1=tp_top,
                   line=dict(color=exit_color, width=2, dash="dot"),
                   row=pos_row, col=pos_col)

    # ============================================================
    # MSS line — horizontal at swing_price from swing_ts to ORIGINAL mss_ts
    # (the actual break bar, not the fill bar — these can differ after retracement)
    # Drawn on:
    #  - the picked asset's 15m panel (so it's visible in strategy-TF context)
    #  - the 5m panel (so it's visible at the drill-down)
    # ============================================================
    mss_break_ts = trade.get("mss_ts_original")
    if pd.isna(mss_break_ts):
        mss_break_ts = mss_ts  # fallback to fill_ts if original wasn't stored

    if have_mss_line:
        # Picked asset's 15m panel (row 1, col 1 or 2)
        picked_15m_col = 1 if measured_asset == "MNQ" else 2
        for r_, c_ in [(1, picked_15m_col), (2, 1)]:
            fig.add_shape(
                type="line",
                x0=swing_ts, x1=mss_break_ts,
                y0=swing_price, y1=swing_price,
                line=dict(color="#fb8c00", width=2),
                row=r_, col=c_,
            )
            fig.add_annotation(
                x=mss_break_ts, y=swing_price,
                text=f"MSS @ {swing_price:.2f}",
                showarrow=False,
                font=dict(size=9, color="#fb8c00"),
                xanchor="left", yanchor="bottom" if direction == "bullish" else "top",
                xshift=4, yshift=4 if direction == "bullish" else -4,
                row=r_, col=c_,
            )
    ...
    # All x-axes share the same window (MSS-4h .. exit+1h)
    ...
    fig.update_yaxes(showgrid=False, showline=True, linecolor="black",
                      tickfont=dict(color="black", size=9), autorange=True)
