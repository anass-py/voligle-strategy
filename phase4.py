...
NEW IN THIS VERSION:
  ...
  - Added:
      ...
      managed_exit_ts                ts
      managed_exit_reason            str: "tp_hit" / "sl_hit" / "opp_smt_kill" / "opp_mss_kill" / "session_end"
...
MEASUREMENT_COLS = [
    "level_id", "smt_birth_ts", "level_type", "direction",
...
def track_outcome(bars_1m, entry_ts, entry_price, sl_price, direction, session_end):
    out = {
            ...
            for anchor in ("before_smt", "after_smt"):
                ...
                for mss_rule in ("wick_break", "close_break"):
                    ...
                    outcome = track_outcome(
                        bars_1m, mss_ts, mss_close, sl_price, direction, end
                    )
                    row.update(outcome)
...
def find_kill_ts(my_row, my_rank, all_smts, mss_lookup):
    """
    For a trade (row): determine the earliest "kill" timestamp from any opposing SMT.
    Rules:
      opposing rank >= mine -> kill at opposing's birth_ts (or my entry if it was alive at entry)
      opposing rank <  mine -> kill at opposing's mss_ts (if exists, in same strategy, after my entry)
                                if no MSS, no kill from this opposing
    ...
    # Opposing SMTs that exist between my_entry and my_session_end
    # (either alive at entry, or born during trade, or has MSS during trade)
    relevant = opps[
    ...
    for _, opp in relevant.iterrows():
        ...
        if opp_rank >= my_rank:
            # Kill triggers at the LATER of opp_birth or my_entry
            # If opp was already alive at my_entry, kill = my_entry (we'd never enter actually,
            # but for during-trade simulation we treat as instant kill at entry).
            # For born-during-trade: kill = opp_birth.
            ...
            # Lower rank -> kill on MSS in same strategy
            ...
            if opp_mss_ts is None:
                continue
            # Only counts if MSS is after our entry (strict — at entry was already filtered)
