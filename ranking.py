...
def simulate_retracement(trades, retrace_r, tp_mult, tp_col, bars_1m):
    """Apply retracement simulation. Trades that don't retrace are skipped."""
    if retrace_r == 0:
        out = trades.copy()
...
def main():
    ...
    for i, (tf, anchor, rule, tp, retrace) in enumerate(combos, 1):
        ...
        sim = simulate_retracement(trimmed, retrace, tp_mult, tp_col, bars_1m)
        if len(sim) == 0:
