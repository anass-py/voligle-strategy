...
st.set_page_config(page_title="SMT Dashboard", layout="wide")
...
def simulate_retraced_entries(trades_df, retrace_r, tp_multiplier, tp_col_name):
    """
    ...
    Trades that never retrace to the target level are EXCLUDED.
    ...
    for _, t in trades_df.iterrows():
        ...
        # Walk to find fill (or SL/kill before fill = no trade)
        try:
...
with r1c2:
    asset_sel = st.selectbox("Asset", ["MNQ","MES","Auto (first MSS)"], key="asset",
                              help="Auto: pick whichever asset's MSS fires first per SMT event. "
                                   "If that asset's entry is skipped (managed mode), fall through to the other.")
with r1c3:
...
if fvg_mode:
    ...
    auto_mode = (asset_sel == "Auto (first MSS)")
    if auto_mode:
        df = df_all[
...
n_skipped_at_entry = 0
if "entry_skipped" in df.columns:
    n_skipped_at_entry = int(df["entry_skipped"].fillna(False).sum())

if hide_opp_live and "opposing_live" in df.columns:
    df = df[df["opposing_live"] != True]

if use_managed and "entry_skipped" in df.columns and not (asset_sel == "Auto (first MSS)" and not fvg_mode):
    df = df[df["entry_skipped"].fillna(False) == False]
...
auto_pick_mode = (asset_sel == "Auto (first MSS)") and not fvg_mode
df = realistic_trim(df, tp_col, single_position=auto_pick_mode)
...
if entry_retrace_R > 0 and not fvg_mode and len(df) > 0:
    ...
    with st.spinner(f"Simulating {entry_retrace_R:.2f}R retraced entries..."):
        df_sim = simulate_retraced_entries(df, entry_retrace_R, tp_multiplier, tp_col)
    n_original = len(df)
    n_filled = len(df_sim)
